...
fn schedule_for(partition: Partition) -> RuntimeSchedule {
    let units = (0..partition.processors)
        ...
        .collect();
    RuntimeSchedule::new(units, 0.15).expect("Phase 9 schedule must be valid")
}
...
fn assert_worker_determinism(partition: Partition, sparse: bool) {
    ...
    for workers in DETERMINISM_WORKERS {
        ...
        .expect("Phase 9 scheduler must start");
        ...
        scheduler
            ...
            .expect("Phase 9 deterministic execution must succeed");
...
fn measure_partition(
    ...
    for sample in 0..warmup {
        ...
        scheduler
            ...
            .expect("Phase 9 warmup must succeed");
    }
    ...
    for sample in 0..samples {
        ...
        let mode = scheduler
            .execute_into(&schedule, &dense_dirty, &mut outputs)
            .expect("Phase 9 dense execution must succeed");
    ...
    for sample in 0..samples {
        ...
        let mode = scheduler
            ...
            .expect("Phase 9 sparse execution must succeed");
        let elapsed = started.elapsed();
    ...
    for sample in 0..samples {
        ...
        let mode = scheduler
            ...
            .expect("Phase 9 idle execution must succeed");
        let elapsed = started.elapsed();
        assert_eq!(mode, ExecutionMode::Sparse);
...
#[test]
#[ignore = "explicit release-mode Phase 9 qualification"]
fn phase9_100k_scalar_dense_sparse_idle_qualification() {
    let samples = env_usize("GC_PHASE9_SAMPLES", DEFAULT_SAMPLES);
    let warmup = env_usize("GC_PHASE9_WARMUP", DEFAULT_WARMUP);
    let workers = env_usize("GC_PHASE9_WORKERS", DEFAULT_MEASUREMENT_WORKERS);
    assert!(
        samples >= DEFAULT_SAMPLES,
        "at least {DEFAULT_SAMPLES} samples are required to measure <0.1% deadline misses"
    );
    assert!(workers > 0, "Phase 9 requires at least one measurement worker");
    ...
    for partition in PARTITIONS {
        ...
        println!(
            concat!(
                "PHASE9_SCALE_RESULT={{\"partition\":\"{}\",\"processors\":{},",
