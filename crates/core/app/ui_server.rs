...
#[derive(Clone)]
/// Configuration for the built-in UI HTTP/WebSocket server.
pub struct UiServerConfig {
    /// Listener address for the UI API (for example `127.0.0.1:7010`).
    pub bind_addr: String,
    /// Target cadence used by the runtime tick loop.
...
fn handle_ws_hub_command<T: Node>(
    ...
    match command {
        ...
        WsHubCommand::Intent { client_id, request_id, intent, include_self_events } => {
...
fn dispatch_ws_batches<T: Node>(engine: &Arc<Mutex<Engine<T>>>, clients: &mut HashMap<u64, WsClientState>, origins: &mut HashMap<EngineTime, WsEventOrigin>) {
        ...
        for (client_id, client) in clients.iter_mut() {
            for (subscription_id, subscription) in client.subscriptions.iter_mut() {
                ...
                for event in batch.events {
                    let skip_for_sender = origins.get(&event.time).is_some_and(|origin| origin.client_id == *client_id && !origin.include_self_events);
                    if !skip_for_sender {
                    ...
                    pending.push((
                        *client_id,
                        WsServerMessage::Batch {
                            subscription_id: subscription_id.clone(),
                            batch: UiEventBatch { from: batch.from, to: batch.to, events: visible_events },
                        },
                    ));
                }
            }
...
fn handle_ws_connection<T: Node>(stream: &mut TcpStream, state: &ServerState<T>, request: &HttpRequest) -> std::io::Result<()> {
    ...
    let key = request.headers.get("sec-websocket-key").ok_or_else(|| Error::new(ErrorKind::InvalidData, "missing sec-websocket-key"))?;
    ...
    state.ws_hub.cmd_tx.send(WsHubCommand::RegisterClient { client_id, outbound: outbound_tx.clone() }).map_err(|_| Error::new(ErrorKind::BrokenPipe, "websocket hub unavailable"))?;
...
fn handle_ws_client_message(message: WsClientMessage, client_id: u64, hub: &WsHubHandle, outbound: &Sender<WsOutbound>) -> bool {
    match message {
        ...
        WsClientMessage::Subscribe { subscription_id, scope, from } => {
            ...
            hub.cmd_tx.send(WsHubCommand::Subscribe { client_id, subscription_id, scope, from }).is_ok()
        }
        WsClientMessage::Unsubscribe { subscription_id } => hub.cmd_tx.send(WsHubCommand::Unsubscribe { client_id, subscription_id }).is_ok(),
        WsClientMessage::Intent { request_id, intent, include_self_events } => {
    ...
    while let Ok(outbound) = outbound_rx.recv() {
        ...
        let write_result = match outbound {
            WsOutbound::Message(message) => match serde_json::to_string(&message) {
                ...
                Err(err) => Err(Error::new(ErrorKind::InvalidData, format!("failed to serialize websocket message: {err}"))),
            },
...
fn is_websocket_upgrade_request(request: &HttpRequest) -> bool {
    request.method.eq_ignore_ascii_case("GET") && header_contains_token(request, "connection", "upgrade") && request.headers.get("upgrade").is_some_and(|value| value.eq_ignore_ascii_case("websocket")) && request.headers.contains_key("sec-websocket-key")
}

fn header_contains_token(request: &HttpRequest, header_name: &str, expected_token: &str) -> bool {
    request.headers.get(header_name).is_some_and(|value| value.split(',').any(|part| part.trim().eq_ignore_ascii_case(expected_token)))
}
...
fn read_ws_frame(stream: &mut TcpStream) -> std::io::Result<Option<WsIncomingFrame>> {
    ...
    if payload_len > WS_MAX_PAYLOAD_BYTES as u64 {
        return Err(Error::new(ErrorKind::InvalidData, "websocket frame exceeds payload limit"));
    }

    if !masked {
        return Err(Error::new(ErrorKind::InvalidData, "client websocket frames must be masked"));
    }
    ...
    match opcode {
        0x1 => {
            let text = std::str::from_utf8(&payload).map_err(|err| Error::new(ErrorKind::InvalidData, format!("invalid websocket text payload: {err}")))?;
        ...
        _ => Err(Error::new(ErrorKind::InvalidData, format!("unsupported websocket opcode: {opcode}"))),
    }
}
...
fn read_http_request(stream: &mut TcpStream) -> std::io::Result<HttpRequest> {
    ...
    Ok(HttpRequest { method, path, headers: parsed_headers, body })
}
...
fn parse_headers(raw_headers: &str) -> std::io::Result<HashMap<String, String>> {
    ...
    let _request_line = lines.next().ok_or_else(|| Error::new(ErrorKind::InvalidData, "missing request line"))?;
    ...
    for line in lines {
        ...
        let (name, value) = line.split_once(':').ok_or_else(|| Error::new(ErrorKind::InvalidData, "malformed header line"))?;
...
fn parse_content_length(headers: &HashMap<String, String>) -> std::io::Result<usize> {
    ...
    value.trim().parse::<usize>().map_err(|err| Error::new(ErrorKind::InvalidData, format!("invalid content-length: {err}")))
}
