...
use crate::app::{configure_loaded_engine, create_new_project_engine, prepare_engine_for_runtime, ProjectLifecycle};
...
pub(crate) fn load_project<T: ProjectLifecycle>(engine: &Arc<Mutex<Engine<T>>>, raw_path: &str) -> Result<(), String> {
    ...
    let mut next_engine = Engine::<T>::load_project_file_with(path.as_str(), T::project_decode_node)
        .map_err(|err| err.to_string())?;
