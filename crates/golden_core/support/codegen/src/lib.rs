...
/// One filesystem root scanned for application nodes.
#[derive(Debug, Clone, Copy)]
pub struct AppNodeSourceRoot<'a> {
    root: &'a Path,
    module_prefix: &'a str,
}

impl<'a> AppNodeSourceRoot<'a> {
    /// Creates a source root. `module_prefix` distinguishes nodes from sibling roots.
    pub const fn new(root: &'a Path, module_prefix: &'a str) -> Self {
        Self { root, module_prefix }
    }
}
...
pub fn generate_app_nodes(src_root: &Path, out_file: &Path) {
    generate_app_nodes_from_roots(&[AppNodeSourceRoot::new(src_root, "")], out_file);
}
...
pub fn generate_app_nodes_from_roots(source_roots: &[AppNodeSourceRoot<'_>], out_file: &Path) {
    ...
    for source_root in source_roots {
        println!("cargo:rerun-if-changed={}", source_root.root.display());

        let mut source_files = Vec::new();
        collect_rs_files(source_root.root, &mut source_files);
