...
impl<T: Node> Engine<T> {
        ...
        F: FnMut(&[NodeId], &mut Vec<NodeId>),
    {
        ...
        let result = (|| {
            for node_id in &due_nodes {
                ...
                *seen += 1;
                ...
                let attempted_callbacks = callback_count.saturating_add(1);
                if attempted_callbacks > self.runtime_limits.max_update_callbacks_per_tick {
                    return Err(EngineRuntimeError::UpdateBudgetExceeded {
                        tick: self.time.tick,
                        callbacks: attempted_callbacks,
                ...
                if let Some(node) = self.nodes.get_mut(node_id) {
                    ...
                    did_update = true;
                    callback_count = attempted_callbacks;
        ...
        // Accepted callbacks keep their queued edits when a later callback is rejected. Restore
        // every extracted scratch allocation before propagating either budget or edit failures;
        // the parameter cache stays in place throughout and therefore needs no recovery path.
        due_nodes.clear();
        due_counts.clear();
