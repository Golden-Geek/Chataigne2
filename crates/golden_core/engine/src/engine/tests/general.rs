...
fn add_node_tree_small_emits_node_created_ops() {
    // N ≤ 8 nodes: use individual NodeCreated ops.
    let mut engine = Engine::new(Folder::new("root".to_string()));
...
// --- Scheduler bucket collection tests ---

#[test]
fn no_due_buckets_zero_per_node_work() {
...
#[test]
fn single_due_bucket_emits_nodes_in_topo_order() {
...
// ── Topological sort Vec stack ──────────────────────────────────────────────

#[test]
fn topo_sort_respects_declared_dependencies() {
...
#[test]
fn topo_sort_detects_dependency_cycle() {
    let root = RuntimeNode::new("root", NodeExecutionRule::passive());
    let mut engine = Engine::new(root);
...
#[test]
fn fixed_step_accumulator_delivers_uniform_delta_times() {
...
#[test]
fn accumulator_fires_correct_tick_count_and_tracks_late_ticks() {
    let step = Duration::from_micros(5_000);
...
#[test]
fn tick_stats_counts_callbacks_fired_and_nodes_due() {
    let root = RuntimeNode::new("root", NodeExecutionRule::passive());
    let mut engine = Engine::new(root);
...
#[test]
fn tick_stats_resets_each_tick() {
    let root = RuntimeNode::new("root", NodeExecutionRule::passive());
    let mut engine = Engine::new(root);
...
#[test]
fn structural_edits_during_stabilization_are_deferred_to_next_tick() {
