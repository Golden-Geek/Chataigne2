//! Canonical runtime values and deterministic cross-domain conversions.
...
/// Stable identifier for a value type shared by registries and runtime values.
...
impl From<&str> for ValueTypeId {
    fn from(value: &str) -> Self {
        Self::new(value)
    }
}

impl From<String> for ValueTypeId {
...
impl Value {
    ...
    pub fn component(&self, component: ValueComponent) -> Option<Self> {
        match (self, component) {
            ...
            (Self::Color(value), ValueComponent::R) => Some(Self::Float(value.red)),
            (Self::Color(value), ValueComponent::G) => Some(Self::Float(value.green)),
            (Self::Color(value), ValueComponent::B) => Some(Self::Float(value.blue)),
            (Self::Color(value), ValueComponent::A) => Some(Self::Float(value.alpha)),
            _ => None,
        }
    }

    pub fn with_component(&self, component: ValueComponent, value: &Value) -> Result<Self, String> {
        ...
        match (self, component) {
            ...
            (Self::Color(current), ValueComponent::R) => Ok(Self::Color(ColorValue { red: value, ..*current })),
    ...
    /// Converts this value to a finite scalar using the canonical lossy rules.
    #[must_use]
    pub fn to_float_lossy(&self) -> f64 {
        match self {
            ...
            Self::Color(value) => value.red,
            Self::Duration(value) => value.as_secs_f64(),
            Self::Array(value) => value.first().map_or(0.0, Value::to_float_lossy),
    ...
    pub(crate) fn to_string_lossy(&self) -> String {
        match self {
            ...
            Self::Array(values) => values.iter().map(Value::to_string_lossy).collect::<Vec<_>>().join(","),
    ...
    pub(crate) fn to_vec2_lossy(&self) -> [f64; 2] {
        match self {
            ...
            Self::Color(value) => [value.red, value.green],
    ...
    pub(crate) fn to_vec3_lossy(&self) -> [f64; 3] {
        match self {
            ...
            Self::Color(value) => [value.red, value.green, value.blue],
            Self::String(value) => parse_components(value)
                .map(|components| {
                    [
                        components[0],
    ...
    /// Converts this value to RGBA using the canonical lossy rules.
    #[must_use]
    pub fn to_color_lossy(&self) -> ColorValue {
        match self {
