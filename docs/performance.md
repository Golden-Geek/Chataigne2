...
- IO parsing, timestamping, reconnect, and device polling run in `crates/io` or app-owned workers.
...
```text
python tools/migration/run_phase9_scale.py --output-dir target/phase9/scale/local
```
...
```text
python tools/migration/run_phase9_graph_scale.py \
  --output-dir target/phase9/graph-scale/local \
  --port 7037
```
...
Phase 9 runs the five-minute multi-client migration soak documented in
[release-readiness.md](release-readiness.md). Longer endurance runs may be selected for an actual
release candidate, but do not block the architecture checkpoint. Reports under `target/` are
