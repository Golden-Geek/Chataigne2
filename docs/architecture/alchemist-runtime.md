...
`apps/chataigne/systems/alchemist`. They build on the app-agnostic `golden_graph`
contracts. Conditions, processors, Inputs, Filters, and Outputs are also Alchemist-owned;
state-machine orchestration, command arbitration, and product UI remain in their focused
Chataigne layers.
...
set, otherwise from `apps/chataigne/resources/formulas/builtin`. If no formula
directory exists, the built-in catalog is empty. Chataigne currently ships
`resources/formulas/builtin/Action.json` and `resources/formulas/builtin/Mapping.json`.
...
Compilation and type solving consume typed-document semantics directly. Formula managed
regions and Chataigne-managed `ValueSet` pipeline builders construct typed
