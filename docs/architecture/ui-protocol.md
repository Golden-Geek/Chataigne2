...
- `crates/golden_core/engine/src/ui_sync.rs` coordinates intent application. Its sibling
  ...
  user-item/dashboard creation and duplication, and `ui_sync/snapshot.rs` owns scoped snapshots,
  node/event projection, and read-only control information. These are engine adapters, not a
  second protocol declaration site.
