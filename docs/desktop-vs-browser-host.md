...
- `submodules/golden_core/crates/host_desktop/src/lib.rs` is the public desktop-host entry point used through the `golden_core::app` facade.
- `submodules/golden_core/crates/host_desktop/src/desktop.rs` launches the built-in UI server and the Tauri window.
...
- `--dev` swaps the desktop host over to the frontend dev server from `src-ui` so `cargo run -- --dev` uses the live Svelte/Vite stack instead of the bundled UI assets.
...
- `submodules/golden_core/crates/host_desktop/src/desktop_commands.rs` owns the Tauri window commands and native file-dialog commands used by the UI.
...
- `submodules/golden_core/crates/transport_server/src/lib.rs` is the public transport-host entry point used through the `golden_core::app` facade.
- `submodules/golden_core/crates/transport_server/src/ui_server.rs` exposes the current HTTP and WebSocket runtime endpoints and serves any bundled frontend assets provided by the app shell.
