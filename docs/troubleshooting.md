...
- Run `python tools/migration/product_manifest.py check` to identify inventory drift; use the
  documented `generate` action only after reviewing the owning source change.
- Regenerate Rust-owned UI bindings with the commands in [ui-extension.md](ui-extension.md).
