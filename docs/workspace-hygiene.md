...
longer needed. The ignored `submodules/` directory was a migration-time import copy; the immutable
source revisions remain recorded under `docs/product/`, so that local copy is not part of a healthy
checkout.
