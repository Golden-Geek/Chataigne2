...
- Rust DTOs are currently defined in `crates/core/src/ui_sync.rs`.
- The public boundary for those DTOs is `crates/protocol/src/lib.rs`.
...
- The built-in HTTP and WebSocket host in `crates/transport_server/src/ui_server.rs`.
