<script lang="ts">
	...
	let lastTitlePointerDown: {
		nodeId: string;
		time: number;
		x: number;
		y: number;
	} | null = null;
	let lastResizePointerDown: {
		nodeId: string;
		time: number;
		x: number;
		y: number;
	...
	const toggleSocketExpanded = (
		...
		expandedSockets = {
			...expandedSockets,
			[key]: !socketExpanded(node, socket, direction)
		};
	};
	...
	let obstacleBuckets = $derived(routingObstacleBuckets(routingObstacles, remPx));
	...
	const routedWirePath = (
		...
		return routeEdgeAroundObstacles(
			start,
			end,
			routeStart,
			routeEnd,
	...
	const draftPath = (): string | null => {
		...
		if (connectionDraft.snapNodeId && connectionDraft.snapSocketId) {
			const end = socketPoint(
				{
					nodeId: connectionDraft.snapNodeId,
					socketId: connectionDraft.snapSocketId
				},
				'input'
			);
		...
		return wirePath(start, {
			x: connectionDraft.endX,
			y: connectionDraft.endY
		});
	};
	...
	const handlePointerEnd = (event: PointerEvent): void => {
		...
		if (nodeDragGesture?.pointerId === event.pointerId) {
			const moves = Object.entries(dragPositions).map(([nodeId, position]): GraphNodeMove => ({
				nodeId,
				position
			}));
			if (moves.length > 0) {
