 ...
 *
 * The current product transport invalidates all graph planes together. The class
 * keeps that conservative policy isolated here until Phase 7 supplies partitioned
 * revisions; it never emits commands or evaluates statechart semantics.
 */
export class StatechartDocumentView {
  ...
  update(snapshot: StatechartPresentationSnapshot): GraphPresentationDocument {
    if (
      snapshot.states === this.#states &&
      snapshot.transitions === this.#transitions
    ) {
