...
export interface WorkbenchSession {
  ...
  getCustomEventPayload<T = unknown>(
    topic: string,
    origin?: NodeId | null,
  ): T | null;
...
export const createWorkbenchSession = (
  ...
  ): string | null => descriptions.getNodeDescription(node);

  const getCustomEventSequence = (
    topic: string,
    origin?: NodeId | null,
  ): number => customEvents.getCustomEventSequence(topic, origin);

  const getCustomEventPayload = <T = unknown>(
    topic: string,
    origin?: NodeId | null,
  ): T | null => customEvents.getCustomEventPayload<T>(topic, origin);
  ...
  const applyHistoryState = (
    nextHistory: UiSnapshot["history"] | undefined,
  ): void => {
  ...
  const commandSuite = createWorkbenchCommandSuite({
    ...
    selectNodes: (nodeIds, selectionMode) =>
      selection.selectNodes(nodeIds, selectionMode),
  ...
  return {
    ...
    setFooterHover,
    clearFooterHover,
    selectNode: (nodeId, selectionMode) =>
      selection.selectNode(nodeId, selectionMode),
    selectNodes: (nodeIds, selectionMode) =>
      selection.selectNodes(nodeIds, selectionMode),
