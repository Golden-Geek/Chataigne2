...
impl Devices {
    ...
    /// Builds an iterator containing only the requested ASIO driver.
    ///
    /// ASIO drivers are exclusive and loading unrelated drivers just to find one
    /// device can make the requested driver unavailable. Exact `DeviceId`
    /// lookups use this constructor instead of walking the installed-driver list.
    pub(super) fn by_name(asio: Arc<sys::Asio>, name: impl Into<String>) -> Result<Self, Error> {
        com::com_initialized();
        Ok(Self {
            asio,
            drivers: vec![name.into()].into_iter(),
            current_driver: None,
        })
    }
}
