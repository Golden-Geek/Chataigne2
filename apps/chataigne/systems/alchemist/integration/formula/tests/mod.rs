...
fn external_formula_creation_exposes_file_parameter_and_loads_formula_file() {
    ...
    let shipped_action_path = std::env::current_dir()
        .expect("current dir should resolve")
        .join("resources")
        .join("formulas")
        .join("builtin")
        .join("Action.json");
    let formula_dir = std::env::temp_dir().join(format!(
        "chataigne2_external_formula_{}",
        uuid::Uuid::new_v4()
    ));
    std::fs::create_dir_all(&formula_dir).expect("temporary formula directory should be creatable");
    let action_path = formula_dir.join("Action.json");
    std::fs::copy(&shipped_action_path, &action_path)
        .expect("shipped Action formula should copy to the test directory");
    ...
    assert!(
        raw_regions.is_empty(),
        "External formula files should retain authored managed-region metadata instead of generated runtime data"
    );
    assert_eq!(
    ...
    drop(engine);
    std::fs::remove_dir_all(formula_dir).ok();
}
...
fn anode_catalog_materializes_and_roundtrips_every_registered_type() {
    ...
    assert_eq!(report_entries.len(), declarations.len());
}
