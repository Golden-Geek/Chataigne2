...
impl ManagedFormulaRuntime {
    ...
    /// A context contributes one lane for each stateful stage or authored graph that retains it.
    #[must_use]
        ...
        match &self.kind {
            ManagedFormulaRuntimeKind::ValuePipeline(runtime) => {
                runtime
                    .typed_stages
                    .as_ref()
                    .map_or(0, ManagedStageChain::retained_state_lane_count)
                    + runtime
                        .graph
                        .as_ref()
                        .map_or(0, GraphManagedExecution::retained_state_lane_count)
            }
            ManagedFormulaRuntimeKind::TriggerPipeline(runtime) => runtime
                .typed_stages
                .as_ref()
