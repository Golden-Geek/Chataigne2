...
#[test]
fn phase8j_modules_round_trip_through_sparse_project_persistence() {
    ...
    engine
        .apply_edits()
        .expect("Phase 8J modules should attach");
    for _ in 0..4 {
        engine
            .apply_edits()
            .expect("Phase 8J defaults should materialize");
    }

    let json = golden_core::app::to_sparse_project_json_pretty(&engine)
        .expect("Phase 8J project should encode");
    let reopened = golden_core::app::from_sparse_project_json::<AppNode>(&json)
        .expect("Phase 8J project should decode");
    ...
    let round_tripped = golden_core::app::to_sparse_project_json_pretty(&reopened)
        .expect("reopened Phase 8J project should encode");
    assert_eq!(
        serde_json::from_str::<serde_json::Value>(&round_tripped).unwrap(),
        serde_json::from_str::<serde_json::Value>(&json).unwrap(),
        "saving, reopening, and saving Phase 8J modules should be stable"
    );
}
