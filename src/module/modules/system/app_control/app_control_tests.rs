...
fn watched_app_file_parameter_populates_values_and_running_control() {
    ...
    set_param(
        &mut engine,
        watched_app_id,
        ParamValue::File("C:/Program Files/Example/Example.exe".to_string()),
    );
    engine
        .apply_edits()
    ...
    assert_eq!(watched_app.node_data().meta.label, "Example");

    let values_folder_id = find_path(&engine, module_id, "values/watched_apps_values/Example")
    ...
    assert_eq!(
        string_param_value(&engine, module_id, "values/watched_apps_values/Example/name"),
...
fn launch_command_watched_app_param_tracks_available_watched_apps() {
    ...
    let variants = watched_app_snapshot
        ...
        .collect::<Vec<_>>();
    assert_eq!(variants, vec!["Example".to_string()]);
    assert_eq!(watched_app_snapshot.value, ParamValue::Enum("Example".to_string()));
}
