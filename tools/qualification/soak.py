...
try:
    from .runtime_scale import command_output, working_tree_sha
except ImportError:
    from runtime_scale import command_output, working_tree_sha


EVIDENCE_ID = "soak.module-network-multiclient-hardware.local"
...
def resolve_output_dir(root: Path, value: Path | None) -> Path:
    ...
    output = (
        ...
        else target / "qualification" / "soak" / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    )
    if output == target or not output.is_relative_to(target):
        raise ValueError("soak output must be below the workspace target directory")
    return output


def validate_browser_report(report: dict[str, Any], duration_ms: int, client_count: int) -> None:
    if report.get("contract") != "chataigne-multiclient-soak-v1":
...
def run(arguments: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the mounted-product soak gate.")
    ...
    if not options.allow_short and options.duration_seconds < MINIMUM_DURATION_SECONDS:
        raise ValueError("release soak qualification requires at least five minutes")
    ...
    hook = root / "tools" / "product-gate" / "hooks" / "soak.ps1"
    ...
    browser_report_path = run_dir / "soak-artifacts" / "soak.browser-report.json"
    ...
    product_logs = (
        run_dir / "soak.product.stdout.log",
        run_dir / "soak.product.stderr.log",
    )
    ...
    report = {
        "schema_version": 1,
        "contract": "chataigne-soak-report-v1",
    ...
    report_path = output / "soak-report.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"Soak report: {report_path.relative_to(root).as_posix()}")
    return 0
