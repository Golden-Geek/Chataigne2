...
def run(arguments: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build and smoke the native application package.")
    ...
    output = (
        ...
        else target / "qualification" / "package" / options.platform / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    )
    if output == target or not output.is_relative_to(target):
        raise ValueError("package qualification output must be below the workspace target directory")
    ...
    smoke = subprocess.run(
        (
            ...
            str(hook),
            "-Id",
            f"{options.platform}-clean-package",
            "-ProductBinary",
            str(product_binary),
            "-Port",
            str(options.port),
    ...
    report_id = f"{options.platform}-clean-package"
    ...
    report = {
        "schema_version": 1,
        "contract": "chataigne-clean-package-report-v1",
        "evidence_id": f"package.{options.platform}.clean",
        "status": "PASS",
    ...
    report_path = output / "clean-package-report.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"Clean-package report: {report_path.relative_to(root).as_posix()}")
    return 0
...
def main() -> int:
    try:
        ...
        print(f"Clean-package smoke failed: {error}", file=sys.stderr)
        return 1
