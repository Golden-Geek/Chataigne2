...
EVIDENCE_ID = "phase9.runtime.scalar-100k.local"
RESULT_PREFIX = "PHASE9_SCALE_RESULT="
...
TEST_COMMAND = (
    ...
    "--test",
    "phase9_scalar_scale",
    "phase9_100k_scalar_dense_sparse_idle_qualification",
    "--",
    "--ignored",
    "--nocapture",
)
...
def parse_results(output: str) -> list[dict[str, Any]]:
    ...
    for line in output.splitlines():
        ...
        try:
            ...
            raise ValueError(f"invalid Phase 9 result JSON: {error}") from error
        if not isinstance(result, dict):
            raise ValueError("Phase 9 result must be a JSON object")
    ...
    if set(partitions) != EXPECTED_PARTITIONS or len(partitions) != len(
        ...
        raise ValueError(
            "Phase 9 output must contain exactly one result for each partition; "
            f"found={partitions}"
        )
...
def working_tree_sha(root: Path) -> str:
    with tempfile.TemporaryDirectory(prefix="chataigne-phase9-index-") as directory:
...
def resolve_output_dir(root: Path, value: Path | None) -> Path:
    ...
    output_dir = (
        ...
        else target_root / "phase9" / "scale" / datetime.now(timezone.utc).strftime(
            "%Y%m%dT%H%M%SZ"
        )
    )
    if not output_dir.is_relative_to(target_root) or output_dir == target_root:
        raise ValueError("Phase 9 evidence output must be a child of the workspace target directory")
    return output_dir
...
def main(arguments: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run and record the Phase 9 100,000-scalar qualification."
    )
    ...
    try:
        ...
        print(f"Phase 9 scale evidence error: {error}", file=sys.stderr)
        return 2

    report_path = output_dir / "phase9-scale-report.json"
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(combined_output, end="" if combined_output.endswith("\n") else "\n")
    print(f"Phase 9 scale report: {report_path.relative_to(root).as_posix()}")
