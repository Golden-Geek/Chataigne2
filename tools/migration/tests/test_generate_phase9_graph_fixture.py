...
from tools.migration.generate_phase9_graph_fixture import FORMULA_LABEL, build_fixture, write_fixture


REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
SOURCE_FIXTURE = REPOSITORY_ROOT / "apps/chataigne/test-samples/test_simple_load.noisette"


class GeneratePhase9GraphFixtureTests(unittest.TestCase):
    ...
    def test_write_fixture_reports_compact_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            output = Path(temporary_directory) / "phase9.noisette"
